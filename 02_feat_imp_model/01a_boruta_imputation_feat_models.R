...
for (modality in 1:length(input_files)) {
  ...
  modality_name <- sub("_ML_cleaned_IMP_.*", "", input_files[modality])
  ...
  for (fold in 1:n_fold) {
    ...
    tic()
    print("run impuatation on training dataset")
    train_data <- as.data.frame(train_data) # needs to be df
    train_data_imp <- missForest(train_data, maxiter = 2000, ntree = 1000)$ximp
    print("##DONE##")
    toc()
    
    tic()
    print("run impuatation on test dataset")
    test_data <- as.data.frame(test_data) # needs to be df
    test_data_imp <- missForest(test_data, maxiter = 2000, ntree = 1000)$ximp
    print("##DONE##")
    toc()
    
    print("Run Boruta feature importance on training fold")
    boruta_imp_dat <- boruta_feat_imp(data = train_data_imp,
                                      target = target,
                                      seed = seed,
                                      max_runs = 2000,
                                      ntree = 1000)
    ...
    boruta_compare <- feature_select_comparison(train_data = train_data_imp,
